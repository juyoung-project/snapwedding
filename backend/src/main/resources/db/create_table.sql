...
CREATE TABLE file_masters
(
    id             BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '파일 ID (PK)',
    file_path      VARCHAR(1024) NOT NULL COMMENT '파일 경로(디렉터리 포함, 논리 경로 또는 실제 경로)',
    file_extension VARCHAR(20) NULL COMMENT '파일 확장자(예: jpg, png, pdf)',
    file_nm        VARCHAR(1024)  NOT NULL COMMENT '원본 파일명',
    file_store_nm  VARCHAR(255)  NOT NULL COMMENT '저장용 파일명(고유 식별자 포함)',
    file_size      BIGINT UNSIGNED NOT NULL COMMENT '파일 크기(Byte)',
    file_mime_type VARCHAR(100)  NULL COMMENT 'MIME 타입(예: image/jpeg)',
    ...
    UNIQUE KEY unique_file_store_nm (file_store_nm)
